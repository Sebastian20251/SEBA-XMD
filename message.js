    ...
    getMenuSections() {
        ...
        for (const [category, commands] of sortedCategories) {
            ...
            const commandList = sortedCommands.map(cmd => {
                ...
                return `︱▶️ ${cmd}${plugin.description ? ` - ${plugin.description}` : ''}`;
            }).join('\n');
...
if (
    m.key.remoteJid === 'status@broadcast' && // only view status
    !m.key.fromMe                          // avoid own status
) {
    // Step 1: view status
    await sock.readMessages([m.key]);

    // Step 2: optional delay (looks natural)
    await new Promise(res => setTimeout(res, 1200));

    // Step 3: like / react to status
    const reacts = ['❤️', '🔥', '👍', '😂']; // random emojis
    ...
    // Step 4: stop further processing
    return;
}
